...
def preprocess_data(merged_df, scaler_path='models/scaler_V1.pkl'):
...
def get_predictions(x, model_path='models/stock_prediction_model_V1_ohneSMOTE.keras'):
...
def add_predictions_to_df(df, y_pred, folder_name='predicted_data'):
    ...
    df.to_csv(os.path.join(folder_name, 'predicted_data_V1.csv'), index=False)
...
feature_indices = np.loadtxt('models/selected_features_V1_ohneSMOTE.txt', dtype=int).tolist()
